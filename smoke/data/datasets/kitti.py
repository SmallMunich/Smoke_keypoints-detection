...
class KITTIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        center = np.array([i / 2 for i in img.size], dtype=np.float32) # center(img.width/2, img.height/2)
        size = np.array([i for i in img.size], dtype=np.float32) # size(img.width, img.height)
