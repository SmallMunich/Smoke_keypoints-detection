...
const double MIN_OVERLAP[3][3] = {{0.7, 0.5, 0.5}, {0.7, 0.25, 0.25}, {0.7, 0.25, 0.25}};
...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      fp_det = fopen((plot_dir + "/stats_" + CLASS_NAMES[c] + "_detection.txt").c_str(), "w");
      if(compute_aos)
        fp_ori = fopen((plot_dir + "/stats_" + CLASS_NAMES[c] + "_orientation.txt").c_str(),"w");
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (eval(gt_dir, result_dir, mail)) {
    ...
    mail->msg((result_dir + "plot").c_str());
  } else {
